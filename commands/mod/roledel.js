...
module.exports = {
  ...
  run: async (bot, message, args) => {
    ...
    if(!message.member.hasPermission(["MANAGE_ROLES"])) return message.channel.send("Vous n'avez pas la permission d'effectuer cette commande!")

    let rMember = message.mentions.members.first() || message.guild.members.cache.get(args[0]);

    if(!rMember) return message.channel.send("Veuillez fournir à un utilisateur de supprimer un rôle de.")
